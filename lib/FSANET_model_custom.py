...
class SSRLayer(Layer):
    ...
    def call(self, inputs):
        x = inputs
        print("Shape into SSR: ", inputs[0].shape)
...
class BaseFSANet(object):
    def __init__(self, image_size,num_classes,stage_num,lambda_d, S_set):
        ...
        logging.debug(f"[INFO] Creating model CUSTOM.")
    ...
    def ssr_G_model_build(self, img_inputs):
        ...
        return Model(inputs=img_inputs,outputs=[feat_s1_pre,feat_s2_pre,feat_s3_pre])

    def ssr_F_model_build(self, feat_dim):
        ...
        return Model(inputs=[input_s1_pre,input_s2_pre,input_s3_pre],outputs=[pred_s1,pred_s2,pred_s3,delta_s1,delta_s2,delta_s3,local_s1,local_s2,local_s3])

    def ssr_FC_model_build(self, feat_dim):
        input_s1_pre = Input((feat_dim,))
        input_s2_pre = Input((feat_dim,))
        input_s3_pre = Input((feat_dim,))
    ...
    def ssr_S_model_build(self, num_primcaps, m_dim):
        ...
        return Model(inputs=[input_s1_preS, input_s2_preS, input_s3_preS],outputs=primcaps)
    ...
    def create_model_flow(self):
        imgs_input = Input(self._input_shape)
        # Build various models
        ssr_G_model = self.ssr_G_model_build(imgs_input)        
        ...
        if self.is_fc_model:
            ssr_F_Cap_model = self.ssr_FC_model_build(self.F_shape)
        else:    
            ssr_F_Cap_model = self.ssr_F_model_build(self.F_shape)        

        # Wire them up
        ssr_G_list = ssr_G_model(imgs_input)
        ...
        pred_pose = SSRLayer(s1=self.stage_num[0], s2=self.stage_num[1], s3=self.stage_num[2], lambda_d=self.lambda_d)(ssr_F_Cap_list)
        
        return Model(inputs=[imgs_input], outputs=pred_pose)   

    def __call__(self):
        logging.debug("Creating model...")
        imgs_input_1 = Input(self._input_shape)
        imgs_input_2 = Input(self._input_shape)

        # because we re-use the same instance `model_flow`,
        # the weights of the network
        # will be shared across the two branches

        model_flow = self.create_model_flow()

        pred_pose_1 = model_flow(imgs_input_1)     
        pred_pose_2 = model_flow(imgs_input_2) 

        merged_pose = Concatenate(axis=1)([pred_pose_1, pred_pose_2])
        # merged_pose = Reshape((6,))(merged_pose)
        print(f"[INFO] Merged pose shape: {merged_pose.shape}")

        return Model(inputs=[imgs_input_1, imgs_input_2], outputs=merged_pose)
...
class BaseCapsuleFSANet(BaseFSANet):
    ...
    def ssr_aggregation_model_build(self, shape_primcaps):
        ...
        return Model(inputs=input_primcaps,outputs=[feat_s1_div,feat_s2_div,feat_s3_div])     
